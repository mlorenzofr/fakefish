...
VM_RUNNING=$(oc -n ${VM_NAMESPACE} get vm ${VM_NAME} --insecure-skip-tls-verify=true -o jsonpath='{.spec.running}')
if [ $? -ne 0 ]; then
...
if [[ ${NUM_VOLUMES} -ge 1 ]]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    oc -n ${VM_NAMESPACE} --insecure-skip-tls-verify=true delete pvc ${VM_NAME}-bootiso
    if [ $? -ne 0 ]; then
